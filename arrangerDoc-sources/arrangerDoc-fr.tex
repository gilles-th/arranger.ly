...
Renvoie un \type{multiMeasasureRest} de longueur soit \type{(ly:make-moment ratio)}(par exemple\\ \type{(mmr 3/4)} pour \type+#{ R4*3 #}+, ou \type{(mmr 2)} pour \type+#{ R1*2 #}+), soit de la longueur de la musique entre les mesures \type{from-bar-num} à \type{to-bar-num} (par exemple \type{(mmr 5 13)} pour un silence remplissant intégralement les mesures 5 à 13). La syntaxe 2 utilise en interne la fonction \type{pos-sub} décrit plus haut.
\blank[1em]
\midaligned{ \tfd ------------}
\page  %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
La fonction associe des lettres aux mesures contenues dans la liste : \type{measures}. Elle convient particulièrement si \type{Score.markFormatter} est de la forme \type{#format-mark-[…]-letters}.\\
...
#(define ((set-dyn fmt0 . fmt-args) arg0 . args)
   (apply format (lst                ;&\tfx\em{& liste des arguments pour format&}&
     ...
     (filter not-procedure? args)))) %&\tfx\em{& syntaxe argo0 / arg1 / arg2… possible &}& 
\stoplily
