...
- Les {\em valeurs négatives} pour \type{i j k …} sont admises. Le code \type{'(5 2 4)} peut aussi s'écrire en \type{4/4}~: \type{'(6 -4)} soit \quotation{mesure 6, moins la valeur d'une noire}. Les valeurs négatives sont le seul moyen d'accéder à une levée en début de morceau~:~position \type{'(1 -4)} pour \type{\partial 4 …}
\blank[0.5em]
...
Noter enfin que la syntaxe \type{`(n ,(ly:make-moment p/q))} peut s'alléger en
...
\blank[0.6em]
{\tfa -} On peut, cependant, facilement créer soi-même des fonctions compatibles \type{apply-to}, avec l'aide d'une fonction "enveloppe" appellée \type{to-set-func}, particulièrement adaptée au changement de propriétés musicales. \type{to-set-func} prend elle-même en paramètre une {\em fonction}, à paramètre 
musical.\\
...
\blank[0.3em]
Pendant l'appel de \type{apply-to}, tous les arguments \type{args} de la sous-fonction \type{set-func} restent identiques et fixés pour tous les instruments contenus dans \type{obj}. Or, il est dans certain cas souhaitable que ces arguments soient au contraire, personnalisables à chaque instrument.\\ Cela sera possible, à la condition d'adopter une nouvelle syntaxe pour l'argument \type{func} de \type{apply-to}, qui sera alors défini comme une paire, avec en 1\high{er} élement, le nom de la sous-fonction, et en 2\high{nd}, une liste, composée des arguments correspondant à chaque instrument.
\page %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Une clef : \type{obj-start-pos} peut optionellement spécifier un point de départ diffèrent du début du morceau~:
\startmylily
...
\godown[-0.55em]
La fonction \type{maj->min} est definie de la manière suivante :
\page
\startmylily
     ...
     (lambda(p) (ly:make-pitch           ;&\em{\small& renvoie le "delta pitch" &}&
       ...
       (+ (ly:pitch-alteration delta)  ;&\em{\small& l'intervalle varie en fonction de p &}&   
          (if (find (same-pitch-as p 'any-octave) special-pitches)
            -1/2  0))))))      ;&\em{\small& same-pitch-as est defini dans checkPitch.ly &}&
\stopmylily
...
L'argument \type{args} est une suite : d'une hauteur (pitch), d'un symbole de percussion, et  optionnellement, d'une barre oblique \type{/}, de séparation avec le groupe suivant.\\
Pour chaque note de \type{music}, la fonction recherche le symbole de percussion correspondant. À défaut d'en trouver, le symbole  \type{percu-sym-def} est donné.\\
L'instrument de percussion est assigné à la propriété \type{'drum-style} de la note.\\
...
\type+\music+ étant la musique référencée par \type{obj}.
Si \type{obj} est une liste, la fonction retourne une liste.\\
Une fonction équivalente a été définie : \type{ca}. Son utilisation permet une syntaxe alternative :
\startmylily
...
\godown[-0.1em]
\hairline
\type{assocDynList} bénéficie d'un \goto{complément d'information}[complément-assocDynList] \at{dans l'addendum II page}[complément-assocDynList].
\hairline
\blank[0.1em]
...
En l'associant à la fonction \in{\type{def!}}[def!]\at{de la page}{}[def!], on peut créer automatiquement des musiques de la forme \type+{s1*…}+, de la même longeur que le morceau.\\
...
Supprime tous les évenements de nom\footnote {Un nom d'événement commence par une majuscule, et se termine par \quotation{Event}. Exemple : {\em 'SlurEvent}} \type{event-sym} \\
