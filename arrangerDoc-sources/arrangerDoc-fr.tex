%%%%%% Documentation de arranger.ly. Version YYYY/MM/DD = 2020/10/05 %%%%%%
...
\item Lilypond 2.20 ou supérieur
...
\startmylily (rm 'cl '(5 2 4) #{ r4 #})\stopmylily
...
\godown[-0.1em]
\hairline
Dans toutes les fonctions qui vont suivre, tout argument se terminant par \type {-pos} (\type {from-pos}, \type {to-pos}, \type {where-pos} etc…) sera du type {\em position}, tel qu'il vient d'être décrit dans tout ce paragraphe. Seront aussi de ce type, les noms tels que \type {pos1}, \type {pos2} etc…
\hairline
...
\blank[0.6em]
{\tfa -} On peut, cependant, facilement créer soi-même des fonctions compatibles \type{apply-to}, avec l'aide d'une fonction "enveloppe" appelée \type{to-set-func}, particulièrement adaptée au changement de propriétés musicales. \type{to-set-func} prend elle-même en paramètre une {\em fonction}, à paramètre 
musical.\\
...
On peut utiliser la fonction \type{def!} décrite auparavant, pour créer une variable qui contiendra les différentes reprises du morceau :\\
\underbar{\sc\Words Exemple 5} :
\blank[0.5em]
\startmylily 
...
Les \type{newnotes} sont des {\em musique}s, mais si à la fois \type{newnotes1} et \type{obj} sont des listes, \type{notes+} est appliqué élément à élément.\\
...
\startfiguretext[right][set-transp-func]{}
 {\vbox{\blank[-0.2em]\externalfigure[set-transp-func-fr][wfactor=345, align=flushright]}}
 {\vbox{\blank[-0.1em]
\setupinterlinespace[medium]
Les 3 instruments sont au départ identiques, puis \type{'II} et \type{'III} sont transposés en mineur.}}
\stopfiguretext
...
#(define (maj->min from-pitch to-pitch)  ;&\em{\small& renvoie la fonction lambda &}&
   (let ((delta (ly:pitch-diff to-pitch from-pitch))
     ...
     (lambda(p) (ly:make-pitch           ;&\em{\small& renvoie le "delta pitch" &}&
       ...
       (+ (ly:pitch-alteration delta)  ;&\em{\small& l'intervalle varie en fonction de p &}&   
...
\index{set-fix-pitch}
Fixe toutes les notes à la hauteur \type{pitch} ou \type{(ly:make-pitch
octave note-index alteration)}.
\type{octave} ne peut être omis (\type{-1} par défaut) que si \type{alteration} l'est aussi (\type{0} par défaut)\\
La fonction équivalente : \type{((set-fix-pitch …) music)} reprend ces mêmes paramètres.
\blank[0.5em]
\midaligned{ \tfd ------------}
\godown[0.3em]
...
\type{args} est une suite de hauteurs de note (pitch) suivie d'un symbole de percussion.\\
...
Cet instrument de percussion est ensuite assigné à la propriété \type{'drum-style} de la note.\\
On peut optionnellement séparer chaque groupe d'arguments avec une barre oblique \type{/}\\
Noter enfin que tout nombre \type{n} sera transformé en \type{(ly:make-pitch -1 n 0)} par la fonction.
\blank[0.2em]
...
\type{range} est de la forme \type{{c, c''}} ou \type{<c, c''>}\\
...
Si au moins 1 des 2 paramètres est une liste (une liste de musiques ou une liste d'instruments),
la fonction retourne une liste de musiques.
\blank[0.8em]
\midaligned{ \tfd ------------}
\godown[0.8em]
...
\type{from-measure} et \type{to-measure} sont des numéros de mesures (des nombres entiers).\\
\type{pos-pat} est une liste de {\em position}s \footnote {Les positions sont définies \in{dans le paragraphe \quotation{positions musicales}}[positions_musicales], \at{page}[positions_musicales].}, avec une lettre, habituellement n, à la place du numéro de mesure.\\
...
\mypar %%%%%
\mbox{\hspace[lilyindent]%
      \externalfigure[exemple09-flute-fr.pdf][wfactor=265, align=flushleft]}
\stopmypar
\godown[0.5em]
...
\type{instru}.\\
Il suffit ensuite de combiner cette voix avec celle des notes et avec \type{global}. L'exemple du début de paragraphe deviendra~:
\godown[-0.4em]
\startlily
(def! 'vlDyn)  ; & \at{voir page}{}[def!]. &
...
Notez enfin que cette méthode rend possible l'insertion de nuances dans des \type{tuplet}s. Un {\em forte} pour la 2\high{ème} croche d'un triolet d'une mesure 5, peut s'obtenir par \type{"(5 12) f"} \footnote{Il y a 12 croches de triolets dans une ronde}. La 3\high{ème} croche par \type{"(5 12 12) f"} ou \type{"(5 6) f"}. La syntaxe avec fractions, elle, n'est utilisable  dans \type{add-dynamics}, que par le biais de variables à inclure dans la chaîne de caractères :
\startlily
#(define frac 1/12)
...
Les \type{pos-dyn-str}s sont de base des chaînes de caractères telles définies dans la fonction \goto{\type{add-dynamics}}[add-dynamics] ci-dessus.\\
...
\stopmylily
L'extraction des nuances pour un instrument donné pourra ensuite se faire en mettant \type{assocDynList} en dernier paramètre des fonctions \type{extract-pos-dyn-str} ou \type{instru-pos-dyn->music}.\\
...
\in{addendum I concernant \type{\global}}[addendum1]\at{page}[addendum1].
\blank[-2em]
%%%%%%%%
\func{metronome}
...
Crée un nom de symbole en ajoutant à la fin d'un nom d'instrument le symbole \type{sym} (suffix).\\
Si \type{to-begin?} est à \type{#t}, \type{sym} devient un préfixe (collé au début).\\
Cette fonction s'applique à un {\em instrument} ou à une liste d'{\em instrument}s.\\
En l'associant à la fonction \in{\type{def!}}[def!]\at{de la page}{}[def!], on peut créer automatiquement des musiques de la forme \type+{s1*…}+, de la même longueur que le morceau.\\
...
Supprime tous les événements de nom\footnote {Un nom d'événement commence par une majuscule, et se termine par \quotation{Event}. Exemple : {\em 'SlurEvent}} \type{event-sym} \\
...
Par défaut, \type{index->string} est la fonction interne \type{index->string-letters} qui renvoie la ou les lettre(s) capitale(s) correspondante(s) à leur index dans l'alphabet, mais en sautant la lettre~\quotation{\type{I}}~:
\blank[0.3em]
...
Si une lettre était déjà définie avant l'appel de \type{def-letters}, la fonction fait précéder la lettre par le caractère \quotation{\type{_}}. Ceci est surtout nécessaire pour les lettres \type{X} et \type{Y}, qui ont \type{0} et \type{1} comme valeur associée dans {\em Lilypond}. Ces  2 lettres deviendront donc {\em toujours} \type{_X} et \type{_Y}. Un message prévient l'utilisateur du changement, sauf si on inclut \type{#f} dans les options (paramètre \type{show-infos?}) :
...
Voici quelques idées d'organisation pour la création d'un arrangement pour une grosse formation. Quelques fonctions sont ici proposées, mais notez bien qu'elles ne font {\em pas} parties de {\em arranger.ly}. Il faudra recopier leurs définitions si on désire les utiliser.\\
...
#(define* (instru->music instru #:optional (clef "treble"))
    (sim (make-clef-set clef)    ;; &\tfx\em{&<< … >>&}&
         global
         (obj->music instru)     ;; &\tfx\em{&notes&}&
         (add-dyn instru)))      &\tfx\em{&%%  nuances&}&
\stopmylily
...
\startlily 
#(define ((set-dyn fmt) arg0 . args) (apply format #f fmt arg0 args))
\stoplily
Le paramètre \type{fmt} est une chaîne de caractères qui pourra se servir des même séquences d'échappement que la fonction scheme \type{format}. Ainsi, par exemple, chaque apparition de \type{~a} dans \type{fmt}, sera remplacée successivement par le paramètre \type{arg0, arg1, arg2 …}, préalablement converti en chaîne de caractères.\\ 
...
\stopmylily
\godown[0.4em]
Cette instruction retourne une liste de chaînes de caractères, donc en principe, pour pouvoir l'inclure comme argument dans \type{assoc-pos-dyn}, il faudrait regrouper en une seule chaîne de caractères, chaque élément de la liste obtenue, avec une barre oblique \type{/} en guise de séparateur. Dans la pratique, \type{assoc-pos-dyn} nous évite ce travail en effectuant elle-même ce formatage quand un argument est une liste.
\page %%%%%%%%%%%%%%%%%%%%%
L'instruction :
\startmylily
assocDynList = #(assoc-pos-dyn
...
\stopmylily
\blank
→ Copier un groupe de nuances restant au sein d'une même mesure\\
On utilise ici une séquence d'échappement supplémentaire \type{~:*} qui permet de revenir au paramètre précédent.
\blank[0.5em]
\startmylily
#(define dyn<> (set-dyn "(~a 8) < / (~:*~a 4 16) > / (~:*~a 2 16) !"))
assocDynList = #(assoc-pos-dyn
...
On utilise la séquence \type{~{ … ~}} qui permet de répéter l'intérieur de la séquence jusqu'à épuisement des éléments de la liste fournie en argument. La séquence \type{~^} elle, n'écrit pas ce qui suit quand le dernier argument est atteint (Dans le cas présent, on évite la dernière barre oblique~\type{/}).
\blank[0.5em]
\startmylily
#(define dyn<> (set-dyn "~{~a < / ~a > / ~a !~^ / ~}"))
assocDynList = #(assoc-pos-dyn
...
\stopmylily
\index{list-offset}
Si une séquence est répétée avec des nuances séparées à chaque fois, d'un nombre de mesures identique (par exemple, un crescendo~\type{<} suivi, 2 mesures après, d'un decrescendo \type{>} se terminant à la 3\high{ème} mesure), l'utilisation de la fonction suivante peut s'avérer judicieuse :
\startmylily
#(define (list-offset l n)
   "Ajoute n à chaque éléments de la liste de nombres l"
   (map (lambda(x)(if (pair? x) (list-offset x n) (+ x n)))
        l))
\stopmylily
Une fonction \type{dyn<>} pourra alors être définie par :
\startmylily
#(define (dyn<> n)
   (apply (set-dyn "~a < / (~a 8) > / (~a 4 16) !")
          (list-offset '(0 2 3) n)))
\stopmylily
