...
To indicate them within the code, we will use the character : (2 points), immediately followed by the duration \type{(8 16 ...)} of the \type{skip} "carrying" eventually the dynamic within the section~\type+\grace+.\\
\vtop{\hbox{
 \hspace[lilyindent]
 \starttable[lT|c|lT|]
...
#(define ((set-dyn fmt0 . fmt-args) arg0 . args)
   (apply format (lst                ;&\tfx\em{& format arguments list&}&
     ...
     (filter not-procedure? args)))) %&\tfx\em{& syntax argo0 / arg1 / arg2… possible &}& 
\stoplily
