%%%%%% English Documentation of arranger.ly. Version YYYY/MM/DD = 2023/09/11 %%%%%%
...
{ % display => The function #1
 ...
 %% workaround :
 \ctxlua{context.index((string.gsub("#1","+", "\\textplus")))}  %%  replace in index, any + character by \textplus
...
\item You need LilyPond 2.24 or higher.
...
\godown[-0.2em]
In order to replace the rest before measure 3 by an A note, one can use a negative number:
\godown[0.4em]
...
To insert the A from measure 2, the following would be then possible:
...
\startmylily 
(rm 'clar '(2 5/4) #{ a'2. #})  ;&\it\rm{& ok with 5/4 : same result as previous code&}&  \stopmylily
...
Extract music in measures range \type{[from-pos to-pos[}. An event will be kept if it begins between these two limits, and his length will be cut if it lasts after \type{to-pos}.\\
...
(rm 'clar 12 (seq (em 'flute 12 15)         ; &\em{&Double the flute&}&
                  #{ r2 r4 #}               ; &\em{&Measure 15&}&
                  (em 'violin '(16 -4) 20)) ; &\em{&Double the violin&}&
\stopmylily 
\blank[1.2em]
\midaligned{ \tfd ------------}
\godown[1.2em]
...
For 2, 3 dots or more, this digit is added after the dot:
\type{4.3} means \type+#{ r4... #}+ (3 dots)

\blank[1em]
\hairline
\type{seq} and \type{seq-r} are now the same function and can be used interchangeably.\\
The name \type{seq-r} remains only for compatibility with previous versions.
\hairline
\page %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The arguments \type{r…} are rational numbers, in the p/q form, with q as a power of two (1 2 4 8…). They indicate the length of each \type{\skip}.\\
In syntax 2, in the \quote {\type{list of lists}} of the 2\high{nd} parameter, the arguments \type{n…} are natural integers, corresponding to those of the command \type+\volta+ ( For example \type+\volta 1,2,3+).\\
The number of repetitions is calculated on the number of \type{r1 r2…} for syntax 1 and on the number of \type{n1 n2…} for syntax 2.
\startmylily (volta-repeat->skip 9 3 5/4) &\em{&or&}& (volta-repeat->skip 9 '((3 1) (5/4 2)) \stopmylily
are in both cases, equivalent to:
\startmylily \repeat volta 2 s1*9 \alternate { \volta 1 s1*3 \volta 2 s4*5 } \stopmylily  \\
\blank
The arguments \type{r…} can alternatively be of type \type{moment}, which allows the use of \type{pos-sub}, an internal function which returns the length between 2 positions. The example below shows it in action, bar 29, and also the function \type{def!}, described \at{page}[def!].\\
\godown[0.6em]
\underbar{\sc\Words Example 5}:
\startmylily 
(def! 'structure)                ; &\em{&same length as \global&}&  
(rm-with 'structure              ; &\em{&adds repeats&}&  
  5 (volta-repeat->skip 9 3 1)                          ; &\em{&in &}&4/4
 29 (volta-repeat->skip (pos-sub 38 29) (* 2 3/4) 3/4)) ; &\em{&in &}&7/8&\em{& and &}&3/4
 50 (volta-repeat->skip (* 9 3/4) '((3/4 1 2) (3/4 3 4 5) (3/4 6 7))) 
                                                        ; &\em{&syntax &}&2&\em{&, in &}&3/4 
...
Returns a \type{multiMeasasureRest} with a duration computed from:\\
\startnarrower[1em]
- a rational (syntax 1) in the p/q form, with q as a power of two (1 2 4 8…) \\
(for example \type{(mmr 3/4)} for \type+#{ R4*3 #}+, or \type{(mmr 2)} for \type+#{ R1*2 #}+),\\ 
- or, for syntax 2, from the music length between 2 bar numbers\\
(for example \type{(mmr 5 13)} to get a rest that completely fulfils bar 5 to 13).\\
\stopnarrower
Syntax 2 internally uses the \type{pos-sub} function described above.
\blank[1.2em]
\midaligned{ \tfd ------------}
\page  %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%
\hairline
Should be presented now in this chapter, the \type{cp} function.\\
\type{cp} (initials of \underbar{c}hange \underbar{p}itch) allows you to modify the pitch of the notes of a piece of music without affecting their rhythm. Conversely, however, it can modify the rhythm of notes without
affecting their pitch. The choice was made to treat the \type{cp} function (and also the 
\type{cp-with} function), in the following section: {\em rhythm} pattern.
\hairline
\godown[2em]  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subject{Manage Lilypond “modes”}
\index{lilypond-mode}
This chapter does not provide new functions as such but shows how to simplify the entry of musical events in the various modes of Lilypond such as \type{\drummode}, \type{\chordmode} etc…\\
In Lilypond, the default mode is not changeable and is fixed to \type{\notemode}:
\startmylily { c d e } &\rm is a shortcut for& \notemode { c d e } \stopmylily
{\em arranger.ly} internally defines a global variable named \type{lilypond-mode}, which can be assigned at any time, to the \type{symbol} of the desired default mode: \type{'drummode} for \type{\drummode}, \type{'chordmode} for \type{\chordmode}, etc…\\
We can carry out this assignment, for example with the \type{set!} scheme function:
\startmylily (set! lilypond-mode 'figuremode) \stopmylily

At the same time, the basic functions of {\em arranger.ly} have a "hidden"\footnote{undocumented} syntax, which allows you to replace an argument of type \type{music} by an argument of type \type {string}, thus making the return value of these functions dependent on the value of \type{lilypond-mode}.\\
If \type{lilypond-mode} is left in \type{'notemode} (this is the default mode in {\em arranger.ly}):\\
\startmylily (rm obj 1 "c d e") &\rm is simply equivalent to& (rm obj 1 #{ c d e #}) \stopmylily
But if \type{lilypond-mode} changes for example to \type{'lyricmode}, this same instruction will now be equivalent to:
\startmylily (rm obj 1 #{ \lyricmode { c d e } #}) \stopmylily
The \type{obj} object must therefore be included in a {\em Lyrics} context and no longer in a {\em Voice} one.
\blank[1em]
Here is the same drum part written without \type{lilypond-mode} then using it:\\
\page %%%%%%%%%%%%%%%
\startlily
%% Traditional method
#(begin
...
\stopfiguretext
\godown[-1em]
It is clear here that the value of \type{lilypond-mode} only affects arguments where a \type{music} type is expected and where a \type{string} type is supplied instead.
\startlily
...
The function associates letters with the bar numbers of the \type{measures} list. It is particularly suitable when \type{Score.rehearsalMarkFormatter} is of the form: 
