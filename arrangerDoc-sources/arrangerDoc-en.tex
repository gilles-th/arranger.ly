%%%%%% English Documentation of arranger.ly. Version YYYY/MM/DD = 2020/10/19 %%%%%%
...
8, 16, etc…) denoting the distance from the beginning of the \type{n}-th bar.\footnote{The \goto{\type{add-dynamics} function}[add-dynamics] \at{page}[add-dynamics], show some pariculars cases where \type{i j k} … are integers but not a power of 2.} 
...
\stopmylily
\type{x-pos} can be used in conjunction with \type{x-rm} for example, in conjunction with the scheme function \type{apply}:
%%% Exemple 5
%\startfiguretext[right][exemple5]{}
\blank
...
Finally, note that this method makes it possible to insert dynamics in \type{tuplet}s. A {\em forte} for the 2nd 8th note of a triplet in a measure 5, can be obtained with \type{"(5 12) f"} \footnote{There are 12 triplet 8th notes in a whole note.}. The 3rd 8th note can be obtained by \type{"(5 12 12) f"} or \type{"(5 6) f"}. The syntax with fractions can only be used, in \type{add-dynamics}, through variables to be included in the string parameter:
\startlily
#(define frac 1/12)
...
\subject{Managing tempo indications}  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\godown[0.2em]
The following 2 functions are used in the \in{addendum I about \type{\global},}[addendum1] \at{page}[addendum1].
\godown[1.5em]
%%%%%%%%
\func{metronome}
...
Insert in \type+\global+ and at the position \type+pos+, the metronome indication \type+\tempo txt+.\\
If a \type{space} number is specified, the \type{txt} {\em markup} is moved horizontally by $+$ or $-$ units to the right or left.\\
...
In addition to the basic functions \type{cons} and \type{append} of {\em\sc GUILE}, we may need the following three or four functions.
\blank[-2em]
%%%%%%%%
...
By associating it to the function \in{\type{def!}}[def!]\at{at page}{}[def!], one can automatically create musics of the form \type+{s1*…}+, with same length as the piece.\\
...
(rm-with 'global 1 #{ \time 3/4 #} /  ;;&\em\tfx{& First, time signatures&}&
                10 #{ \time 5/8 #} /
                20 #{ \time 4/4 #})
(cut-end 'global 70)                  ;;&\em\tfx{& Cuts what's beyond&}&
(x-rm 'global #{ \mark\default #}     ;;&\em\tfx{& Marks&}&
         10 20 30 40 50 60)
...
(rm-with 'global 1 #{ \key c \major #} /  ;;&\em\tfx{& Key signatures&}&
                20 #{ \key c \minor #} /
                30 #{ \key c \major #})
(x-rm 'global #{ \bar "||" #} 20 30 60)   ;;&\em\tfx{& Bars&}&
(rm-with 'global 1 #{ \markLengthOn #})   ;;&\em\tfx{& Miscellaneous &}&  
(rm 'global 70 #{ \bar "|." #})           ;;&\em\tfx{& …the final touch&}&
)                                         %%&\em\tfx{& End \global&}&
...
Here are some ideas for organisation when creating an arrangement for a large orchestral ensemble. Some functions are suggested here, but please note that they are {\em not} part of {\em arranger.ly}. To use them, you will have to copy their definitions.\\
...
In the following example, the text will be left-aligned in the score and right-aligned in the euphonium part:\type{ (rm 'euph 5 (txt "Emphasized !" UP (if (part? 'score) LEFT RIGHT))) }\\

%%%%%%%%%
\index{instru->music}
\reference[instru->music]

{\tfa\bf→} {\bf Separate parts: code lightening - function \type{instru->music}}\\
   ...
   (sim
     ...
     (part-combine                ; &\tfx\em{&\partCombine&}&
       (sim                       ; &\tfx\em{&upper voice&}&
         partCombineAutomatic     ; &\tfx\em{&set default mode&}&
         dynamicUp                ; &\tfx\em{&dynamics direction UP&}&
         (add-dyn (list 'xor instru1 instru2))
         (obj->music instru1))
...
The sequence used now is: \type{~{ ... ~}} It allows you to repeat the sequence until the elements of the list provided as an argument are exhausted. The sequence \type{~^}, for its part, does not write what follows it when the last element is reached (in our case, we avoid the last slash~\type{/}).
\startmylily
...
\stopmylily
\index{list-offset}
If a sequence is repeated with dynamics separated each time with the same number of bars (for example, a crescendo~\type{<} followed, 2 bars later, by a decrescendo \type{>} ending at the 3\high{rd} bar), it may be useful to use the following function:
\startmylily
#(define (list-offset l n)
   "Adds n to each element of the list of numbers l"
