...
{\em arranger.ly provides an environment facilitating musical arrangement.}
\footnote{To arrange herein means to re-orchestrate an original instrumentation.}
A set of functions enables quick re-orchestration of a piece of music, using 
 minimal and reusable music encoding.
\blank[0.7em]
One of the main aspects of {\em arranger.ly} concerns the locating system of
musical positions, which is  now based on {\em bar numbers}\footnote {Lilypond
uses a system based on {\em moment}s : \type{(ly:make-moment 5/4)} for example.}.
The arranger's workflow is made more flexible : rather than entering music
expressions instrument by instrument in a linear fashion, it becomes possible
to work as the ideas go by -- first deal with the melody, then accompaniment,
then the bass, etc.
\blank[0.7em]
Typically, the user first declares a list of instruments. {\em arranger.ly}
takes care of initializing each instrument with empty measures. Then, in a
single command, the user can insert a music fragment in several instruments
and positions, as well as \quotation{copy-paste} entire music sections in one
line of code.

Functions allow for octave transposing and octave doubling, specifying patterns
for repeated rhythms or articulations, distributing the notes to various instruments
in a succession of chords, inverting chords,~\dots, so as never to repeat information.

All these functions can be directly used from Scheme, which makes for lighter
syntax (no backslash before variable names) and easier editing of instrument lists.
...
It is easiest to put these 6 files in the same folder alongside with
{\em arranger.ly}, and call LilyPond with option \type{--include=myfolder}.
Only the following line should then be added at the top of your \type{.ly} file:\\
...
In this document, we call {\em instrument} any Scheme symbol referencing
...
optional upbeat). By contrast, in the following text, {\em music} more generally refers to
...
Assuming \type{\global} was defined using:
\startlily
...
is direct editing of the \type{\global} variable, as shown in
\goto{addendum I}[addendum1] at \at{page}[addendum1].
...
To count measures, \type{init} takes into account manual overrides applied to
properties of the \type{Score} context and the \type{Timing} object, such as
\type{measurePosition}, \type{measureLength}, \type{currentBarNumber}, as well
as the \type{\partial}, \type{\cadenzaOn} and \type{\cadenzaOff} commands. If
\type{\partial} is placed at the very beginning of the piece, \type{init} even
adds a rest with same duration as the pickup to all the instruments.
...
\stopmylily
prints the number of quarter notes elapsed from music start for measures 1, 2,
3, 4 and 50:
\startmylily
...
Optional reading is chapter 8 from this file's documentation at~:\crlf %%%% TODO: missing URL?
...
\mbox{a list of {\em instrument}s : \type{'(clar tptte sax)}}
...
it is a {\em music position} as defined in the next paragraph (\at{page}[positions_musicales]).

- \type{repla} is a {\em music} or a list of {\em music}s, but syntax with \type{unquote '} is
valid~:~\type{'(musicA musicB musicC…)}. %% TODO: unclear
...
obtained after performing the replacement. This new value is automatically assigned
to the appropriate symbol.

- If \type{obj} is a list of instruments or musics, \type{rm} returns a list of musics.
\blank[0.7cm]
\startfiguretext[right][exemple2]{}
{\vbox{\blank[-1.1em]\externalfigure[exemple02.pdf][wfactor=175, align=flushright]}}
\underbar{\sc\Words example 2}
\blank[0.7cm]
\startlily
global = { s1*4 \bar "|." } 
...
By default, \type{rm} accounts for the entire music given in \type{repla}.
It is however possible to extract only part of it by specifying the optional
parameter \type{repla-extra-pos}. The principle is as follows:
...
(\type{repla-extra-pos} relates to \type{repla}). Typically, \type{obj} is a
{\em music} rather than an {\em instrument} and the return value is used.

In \in{example}[exemple3], we change the F note at bar 6
into an E flat, assigning the result to another instrument, a sax.
...
\hairline
\blank
- In case \type{obj} is a list of instruments, any element of this list may in turn
be a list of instruments. Thus,
\startmylily
...
Thus, in a \type{4/4} beat, \type{'(5 2 4)} is a position, located in measure 5,
after a half note (\type{2}) and a quarter note (\type{4}), that is, fifth measure,
fourth beat.

Any \type{n} lower than the \type{measure1-number} passed to \type{init}, which
defaults to 1, will be silently transformed into that number. In the previous
example, \type{'(0 2 4)} points to the same location as \type{'(1 2 4)}.

{\em Negative} values for \type{i j k …} are allowed. In a 4/4 beat,
\type{'(5 2 4)} is the same position as \type{'(6 -4)}, which reads
\quotation{One quarter note before measure 6.}. This is the only way to access
a pickup at the start of the piece: \type{'(6 -4)} is the beginning of a tune
starting with \type{\partial 4 …}.

Any note held at the beginning of the replacement is appropriately shortened by
\type{rm}.
...
{c2. r4}
\stopmylily
as the clarinet's fifth measure, turning the whole C note into a dotted half note.
...
\hairline
\blank
This is why, in our example,
\startmylily
(rm 'flute '(5 2 4) #{ c''4 #})
\stopmylily
would trigger a warning resembling:
\startmylily
&\it\rm{&«warning: barcheck failed at: 1/2
                      r\longa R1*1000000000»&}&
\stopmylily
(This line originates from the {\em arranger.ly} file.)

The solution is:
\startmylily
(rm 'flute 5 #{ r2 r4 c''4 #}) ;&\em{&rests written out by hand&}&
\stopmylily
%\crlf % <- ne marche pas ici
\blank[1em]

This last example demonstrate the use of positions with the \type{\cadenzaOn}
command.
...
Here, this would translate into
\startmylily
...
\type{from-pos}, \type{to-pos}, \type{where-pos}, etc.) shall be positions as
